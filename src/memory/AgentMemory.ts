...
const AUTO_EXTRACT_RULES: ExtractionRule[] = [
  {
...
export class AgentMemory implements vscode.Disposable {
  private static instance: AgentMemory | undefined;

  private readonly context: vscode.ExtensionContext;

  /** In-memory cache: agentName -> facts */
  private cache: Map<string, AgentFact[]> = new Map();
  ...
  private static readonly FLUSH_DELAY_MS = 2_000;

  /** Backend API base URL — set via setBackendConfig() when user is authenticated */
  private _backendApiBase: string | undefined;

  /** Bearer auth token for backend requests */
  private _backendAuthToken: string | undefined;
   ...
   */
  public static getInstance(context?: vscode.ExtensionContext): AgentMemory {
    if (!AgentMemory.instance) {
      if (!context) {
        throw new Error(
          'AgentMemory must be initialized with ExtensionContext on first call'
        );
      }
   ...
   */
  setBackendConfig(apiBase: string, authToken: string): void {
   ...
   */
  async syncFromBackend(agent: string): Promise<void> {
    if (!this._backendApiBase || !this._backendAuthToken) {
      return;
    }
    try {
      const res = await fetch(
        `${this._backendApiBase}/api/memory/${encodeURIComponent(agent)}`,
        {
      ...
      if (!res.ok) {
        console.debug(
          `AgentMemory: backend sync skipped for ${agent} (HTTP ${res.status})`
        );
        return;
      }
      const data = (await res.json()) as { core?: Record<string, string> };
   ...
   */
  private _pushToBackend(agent: string, key: string, value: string): void {
    if (!this._backendApiBase || !this._backendAuthToken) {
      return;
    }
    fetch(
      `${this._backendApiBase}/api/memory/${encodeURIComponent(
        agent
      )}/core/${encodeURIComponent(key)}`,
      {
        method: 'PUT',
        headers: {
      ...
      console.debug(
        'AgentMemory: backend push failed (non-critical):',
        err?.message ?? err
      );
    });
  }
  ...
  async addFact(
    ...
    const maxFacts = config.get<number>('maxMemoriesPerAgent', 500);
   ...
   */
  async getFacts(agent: string): Promise<AgentFact[]> {
    const facts = await this.loadAgent(agent);
    return [...facts];
  }
   ...
   */
  async deleteFact(agent: string, key: string): Promise<void> {
    const facts = await this.loadAgent(agent);
   ...
   */
  async searchFacts(agent: string, query: string): Promise<AgentFact[]> {
    const facts = await this.loadAgent(agent);
   ...
   */
  async getRelevantFacts(agent: string, message: string): Promise<AgentFact[]> {
    const facts = await this.loadAgent(agent);
    if (facts.length === 0) {
      return [];
    }
    ...
    const scored: Array<{ fact: AgentFact; score: number }> = [];
   ...
   */
  async extractFromAssistantResponse(
    agent: string,
    response: string
  ): Promise<AgentFact[]> {
    const config = vscode.workspace.getConfiguration('helix.memory');
    if (
      ...
      !config.get<boolean>('autoLearn', true)
    ) {
      return [];
    }

    const extracted: AgentFact[] = [];
  ...
  async buildContextInjection(
    ...
    const lines = relevant.map(
      f => `- ${f.value} (${f.source}, ${f.createdAt})`
    );
  ...
  private ensureStorageDir(): void {
    ...
    try {
      ...
      console.warn(
        `AgentMemory: failed to create storage directory ${dir}:`,
        err
      );
    }
  }
   ...
   */
  private agentFileName(agent: string): string {
    const safe = agent
      .toLowerCase()
      .replace(/[^a-z0-9_-]/g, '_')
      .slice(0, 64);
   ...
   */
  private agentFilePath(agent: string): string {
   ...
   */
  private async loadAgent(agent: string): Promise<AgentFact[]> {
    ...
    try {
      if (fs.existsSync(filePath)) {
        ...
        const parsed = JSON.parse(raw) as FactFileContents;
  ...
  private async flushToDisk(): Promise<void> {
    ...
    for (const agent of this.dirty) {
      ...
      const filePath = this.agentFilePath(agent);
      const contents: FactFileContents = {
        version: 1,
        agent,
        facts,
      };
 ...
 */
export function getAgentMemory(context?: vscode.ExtensionContext): AgentMemory {
