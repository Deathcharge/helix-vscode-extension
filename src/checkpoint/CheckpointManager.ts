...
/** Metadata for a single file within a checkpoint snapshot. */
export interface CheckpointFile {
  /** Absolute filesystem path */
  path: string;
  /** File content at the time of the checkpoint */
  content: string;
  /** VS Code languageId (e.g. "typescript", "python") */
  languageId: string;
}

/** On-disk format for a checkpoint snapshot (the per-checkpoint JSON file). */
interface CheckpointDiskPayload {
  id: string;
  timestamp: number;
  description: string;
...
/** In-memory / index representation of a checkpoint (no file content). */
export interface Checkpoint {
  id: string;
  taskId: string;
  timestamp: Date;
  description: string;
  /** Absolute paths of files included in this checkpoint */
  filePaths: string[];
  /** Approximate size in bytes of the checkpoint JSON on disk */
  sizeBytes: number;
  metadata?: {
    agent?: string;
    mode?: string;
...
// ---------------------------------------------------------------------------
// Serialisation helpers (Date <-> string for JSON round-trip)
// ---------------------------------------------------------------------------
...
export class CheckpointManager {
  ...
  private eventEmitter: vscode.EventEmitter<TaskHistory[]> =
    new vscode.EventEmitter<TaskHistory[]>();

  /** Maximum number of checkpoints to retain. */
  private static readonly MAX_CHECKPOINTS = 10;

  /** Maximum total disk space for all checkpoint snapshot files (50 MB). */
  private static readonly MAX_TOTAL_BYTES = 50 * 1024 * 1024;
  ...
  public static getInstance(
    ...
    if (!CheckpointManager.instance) {
      if (!context) {
        throw new Error(
          'CheckpointManager requires ExtensionContext on first initialization'
        );
      }
  ...
  private getCheckpointsDirUri(): vscode.Uri {
    return vscode.Uri.joinPath(this.context.globalStorageUri, 'checkpoints');
  }

  private getIndexFileUri(): vscode.Uri {
    return vscode.Uri.joinPath(this.getCheckpointsDirUri(), 'index.json');
  }

  private getSnapshotFileUri(checkpointId: string): vscode.Uri {
    return vscode.Uri.joinPath(
      this.getCheckpointsDirUri(),
      `${checkpointId}.json`
    );
  }
  ...
  private async loadIndex(): Promise<void> {
    try {
      ...
      const index: IndexFile = JSON.parse(json) as IndexFile;
  ...
  private async saveIndex(): Promise<void> {
    try {
      ...
      const checkpoints: CheckpointJSON[] = Array.from(
      ...
      const tasks: TaskHistoryJSON[] = Array.from(
        this.taskHistory.values()
      ).map(t => ({
        taskId: t.taskId,
        name: t.name,
      ...
      const index: IndexFile = { checkpoints, tasks };
      const json = JSON.stringify(index, null, 2);
      await vscode.workspace.fs.writeFile(
        this.getIndexFileUri(),
   ...
   */
  private async migrateLegacyGlobalState(): Promise<void> {
    try {
      const oldCheckpoints =
        this.context.globalState.get<Record<string, unknown>[]>('checkpoints');
      const oldTasks =
        this.context.globalState.get<Record<string, unknown>[]>('taskHistory');
      ...
      if (oldCheckpoints) {
        for (const raw of oldCheckpoints) {
          ...
          if (id && !this.checkpoints.has(id)) {
            ...
            // stays consistent.
            const snapshotObj = raw['snapshot'] as
            ...
            this.checkpoints.set(id, {
              ...
              metadata: raw['metadata'] as Checkpoint['metadata'],
            });
            migrated = true;
          }
        }
        ...
        for (const raw of oldTasks) {
          ...
          if (taskId && !this.taskHistory.has(taskId)) {
            this.taskHistory.set(taskId, {
              ...
              endTime: raw['endTime']
                ? new Date(raw['endTime'] as string)
                : undefined,
              status: (raw['status'] as TaskHistory['status']) ?? 'completed',
              checkpoints: (raw['checkpoints'] as string[]) ?? [],
              metadata: raw['metadata'] as TaskHistory['metadata'],
            });
            migrated = true;
          }
        }
      ...
      console.log(
        'CheckpointManager: migrated legacy globalState data to filesystem'
      );
    } catch (err) {
      console.warn(
        'CheckpointManager: failed to migrate legacy globalState:',
        err
      );
    }
  }
  ...
  private async readSnapshotFromDisk(
    checkpointId: string
  ): Promise<CheckpointDiskPayload | null> {
    try {
      const fileUri = this.getSnapshotFileUri(checkpointId);
      const bytes = await vscode.workspace.fs.readFile(fileUri);
      const json = Buffer.from(bytes).toString('utf-8');
      return JSON.parse(json) as CheckpointDiskPayload;
    } catch {
      return null;
    }
  }
   ...
   */
  private async deleteSnapshotFromDisk(checkpointId: string): Promise<void> {
    try {
      const fileUri = this.getSnapshotFileUri(checkpointId);
   ...
   * Start a new task
   */
  public startTask(name: string, metadata?: TaskHistory['metadata']): string {
   ...
   * Get active task
   */
  public getActiveTask(): TaskHistory | undefined {
    if (!this.activeTaskId) {
      return undefined;
    }
   ...
   * Get task by ID
   */
  public getTask(taskId: string): TaskHistory | undefined {
  ...
  public getTaskHistory(limit?: number): TaskHistory[] {
    const tasks = Array.from(this.taskHistory.values()).sort(
   ...
   * captured.
   */
  private async snapshotOpenEditors(): Promise<CheckpointFile[]> {
    ...
    for (const doc of vscode.workspace.textDocuments) {
      ...
      try {
        ...
        console.warn(
          `CheckpointManager: could not read open document ${fsPath}:`,
          err
        );
      }
    }
  ...
  public async restoreCheckpoint(checkpointId: string): Promise<boolean> {
    ...
    const result = await vscode.window.showWarningMessage(
      `Restore ${fileCount} file(s) to checkpoint "${checkpoint.description}"?\n\n` +
    ...
    if (!payload) {
      vscode.window.showErrorMessage(
        `Checkpoint data file for "${checkpointId}" is missing or corrupted.`
      );
      return false;
    }
    ...
    if (payload.files.length === 0) {
      vscode.window.showInformationMessage(
        'Checkpoint contains no files to restore.'
      );
      return true;
    }
      ...
      async progress => {
        for (let i = 0; i < payload.files.length; i++) {
          ...
          try {
            ...
            console.warn(
              `CheckpointManager: failed to restore file ${file.path}:`,
              err
            );
    ...
    if (failed > 0) {
      ...
      vscode.window.showWarningMessage(
        `Restored ${restored} file(s). ${failed} file(s) could not be restored:\n${failList}${more}`
      );
    } else {
      vscode.window.showInformationMessage(
        `Restored ${restored} file(s) from checkpoint: ${checkpoint.description}`
      );
    }
   ...
   */
  private async enforceCheckpointLimits(): Promise<void> {
   ...
   */
  public getCheckpointsForTask(taskId: string): Checkpoint[] {
    const task = this.taskHistory.get(taskId);
    if (!task) {
      return [];
    }
   ...
   */
  public getAllCheckpoints(limit?: number): Checkpoint[] {
    const sorted = Array.from(this.checkpoints.values()).sort(
   ...
   */
  public async autoCleanup(): Promise<void> {
    await this.ensureIndexLoaded();
    ...
    if (cleanedCount > 0) {
      await this.saveIndex();
      console.log(
        `CheckpointManager: auto-cleanup removed ${cleanedCount} old task(s)`
      );
    }
  }
  ...
  public getStatistics(): {
    ...
    return {
      ...
      totalOperations: checkpoints.reduce(
        (sum, cp) => sum + (cp.metadata?.operationCount ?? 0),
        0
      ),
   ...
   * Export task history
   */
  public exportTaskHistory(taskId?: string): string {
    if (taskId) {
      const task = this.taskHistory.get(taskId);
      if (!task) {
   ...
   */
  public readonly onDidChangeTaskHistory = this.eventEmitter.event;
}
