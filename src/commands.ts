...
import {
  ...
  getBranchCommits,
  getGitBlame,
} from './utils/git-integration';

export function registerCommands(
  context: vscode.ExtensionContext,
  extension: HelixVSCodeExtension
): void {
  // Core panel commands
  const openAgentPanelCommand = vscode.commands.registerCommand(
  ...
  const openCoordinationDashboardCommand = vscode.commands.registerCommand(
  ...
  const openSpiralBuilderCommand = vscode.commands.registerCommand(
  ...
  const connectToAgentCommand = vscode.commands.registerCommand(
  ...
  const quickActionCommand = vscode.commands.registerCommand(
    'helix.quickAction',
    async (action: string) => {
      switch (action) {
        case 'createWorkflow':
          await (async (extension: HelixVSCodeExtension) => {
            ...
            if (!selectedSteps || selectedSteps.length === 0) {
              vscode.window.showWarningMessage(
                'No steps selected for workflow'
              );
              return;
            }
            ...
            try {
              ...
              vscode.window.showInformationMessage(
                `Workflow "${name}" created with ${selectedSteps.length} steps. ID: ${workflowId}`
              );
            ...
            try {
              ...
              if (selected) {
                ...
                if (confirm === 'Install') {
                  ...
                  vscode.window.showInformationMessage(
                    `Agent "${selected.id}" installed successfully!`
                  );
                }
              }
            } catch (error) {
              vscode.window.showErrorMessage(
                `Failed to browse marketplace: ${error}`
              );
            }
          })(extension);
          break;
          ...
          await (async (extension: HelixVSCodeExtension) => {
            try {
              ...
              if (currentVersion === latestVersion) {
                vscode.window.showInformationMessage(
                  `✓ Helix is up to date (v${currentVersion})`
                );
              } else {
      ...
      try {
        ...
        await vscode.window.withProgress(
          ...
          async (progress, token) => {
            ...
            try {
              // Try real API first
              const result = await extension.apiService.agenticAnalyzeCode(
                code,
                language
              );
        ...
        panel.webview.html = getCodeAnalysisHtml(
          analysis,
  ...
  const generateTestsCommand = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      let selectedFramework: string = frameworks[0] ?? 'Generic';
      ...
      try {
        ...
        await vscode.window.withProgress(
          ...
          async (progress, token) => {
            ...
            try {
              // Try real API first
              const result = await extension.apiService.agenticGenerateTests(
                code,
                language,
                selectedFramework,
                fileName
        ...
        const action = await vscode.window.showInformationMessage(
          `Tests generated for ${testFileBaseName}`,
          'Create Test File',
          'Copy to Clipboard',
          'Preview'
        );
        ...
        if (action === 'Create Test File') {
          ...
          vscode.window.showInformationMessage(
            `Test file created: ${testFileBaseName}`
          );
    ...
    async () => {
      ...
      if (selection.isEmpty) {
        return vscode.window.showWarningMessage(
          'Please select code to refactor'
        );
      }
      ...
      try {
        ...
        await vscode.window.withProgress(
          {
            location: vscode.ProgressLocation.Notification,
            title: `Applying ${selected.label.replace(
              /\$\([^)]+\)\s*/,
              ''
            )}...`,
            cancellable: true,
          ...
          async (progress, token) => {
            ...
            try {
              // Try real API first
              const result = await extension.apiService.agenticRefactorCode(
                code,
                selected.value
              );
              ...
              progress.report({ increment: 100, message: 'Done!' });
              refactored = generateRefactoredCode(
                code,
                language,
                selected.value
              );
  ...
  const generateDocumentationCommand = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      const availableFormats = formatOptions.filter(
        f => f.languages.includes('all') || f.languages.includes(language)
      );
      ...
      try {
        ...
        if (action === 'Insert Above Selection') {
          ...
          const docLanguage =
            selected.value === 'readme' || selected.value === 'api'
              ? 'markdown'
              : language;
          const doc = await vscode.workspace.openTextDocument({
            content: docs,
        ...
        await vscode.window.withProgress(
          ...
          async (progress, token) => {
            ...
            try {
              // Try real API first
              const result =
                await extension.apiService.agenticExplainCode(code);
              ...
              explanation = generateBasicExplanation(
                code,
                language,
                selected.value
              );
  ...
  const toggleCoordinationMonitoringCommand = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      await config.update(
        'enableCoordinationMonitoring',
        !currentValue,
        vscode.ConfigurationTarget.Global
      );
      vscode.window.showInformationMessage(
        `Coordination monitoring ${currentValue ? 'disabled' : 'enabled'}`
      );
    }
  );
  context.subscriptions.push(toggleCoordinationMonitoringCommand);

  const showCoordinationDetailsCommand = vscode.commands.registerCommand(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('helix.login', async () => {
    ...
    vscode.commands.registerCommand('helix.createAccount', () => {
      vscode.env.openExternal(
        vscode.Uri.parse('https://helixcollective.io/signup')
      );
    })
  );
  ...
  const toggleInlineCompletionsCommand = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      vscode.window.showInformationMessage(
        `Helix inline completions ${!current ? 'enabled' : 'disabled'}`
      );
    }
  );
      ...
      try {
        ...
        await vscode.window.withProgress(
          ...
          async (progress, token) => {
            ...
            if (!diff.trim()) {
              try {
                diff = await getGitDiff(false);
              } catch (err) {
                const msg = err instanceof Error ? err.message : String(err);
                vscode.window.showWarningMessage(
                  `Could not read git diff: ${msg}`
                );
                return;
              }
            }
            ...
            const style =
              vscode.workspace
                ...
                .get<string>('git.commitMessageStyle') || 'conventional';
            ...
            if (gitExtension) {
              ...
              if (repo) {
                ...
                vscode.window.showInformationMessage(
                  'Commit message generated and placed in SCM input box.'
                );
              } else {
                // No repository — copy to clipboard as fallback
                await vscode.env.clipboard.writeText(commitMessage.trim());
                vscode.window.showInformationMessage(
                  'No git repository found in SCM. Commit message copied to clipboard.'
                );
              }
            } else {
              await vscode.env.clipboard.writeText(commitMessage.trim());
              vscode.window.showInformationMessage(
                'Git extension not available. Commit message copied to clipboard.'
              );
            }
          }
        );
  ...
  const generatePRDescriptionCommand = vscode.commands.registerCommand(
    'helix.generatePRDescription',
    async () => {
      try {
        await vscode.window.withProgress(
          ...
          async (progress, token) => {
            ...
            try {
              branchInfo = await getBranchCommits();
            } catch (err) {
              const msg = err instanceof Error ? err.message : String(err);
              vscode.window.showWarningMessage(
                `Could not read branch history: ${msg}`
              );
              return;
            }
            ...
            if (!branchInfo.trim()) {
              vscode.window.showInformationMessage(
                'No commits found on this branch relative to main/master.'
              );
              return;
            }
            ...
            const prompt =
              `Generate a pull request description for the following branch changes. ` +
        ...
        vscode.window.showErrorMessage(
          `Failed to generate PR description: ${msg}`
        );
      }
    }
  );
  ...
  const explainGitBlameCommand = vscode.commands.registerCommand(
    'helix.explainGitBlame',
    async () => {
      try {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
          vscode.window.showWarningMessage(
            'Open a file and place your cursor on a line to explain its git blame.'
          );
          return;
        }
        ...
        await vscode.window.withProgress(
          ...
          async (progress, token) => {
            ...
            try {
              blameOutput = await getGitBlame(filePath, line);
            } catch (err) {
              const msg = err instanceof Error ? err.message : String(err);
              vscode.window.showWarningMessage(
                `Could not get git blame: ${msg}`
              );
              return;
            }
            ...
            if (!blameOutput.trim()) {
              vscode.window.showInformationMessage(
                'No blame information available for this line.'
              );
              return;
            }
            ...
            const prompt =
              `Explain why this line was changed based on the git blame information below. ` +
...
function getWorkflowEditorHtml(
...
interface CodeAnalysis {
  ...
  metrics: { complexity: string; maintainability: string; testability: string };
}

function analyzeCodeContent(code: string, language: string): CodeAnalysis {
  ...
  const functionCount = (
    code.match(/function\s|def\s|fn\s|func\s|=>\s*\{/g) || []
  ).length;
  if (functionCount > 15) {
    suggestions.push({
      type: 'suggestion',
...
function getCodeAnalysisHtml(analysis: CodeAnalysis, language: string): string {
  const issuesHtml =
    analysis.issues
      .map(
        issue => `
    <div class="issue ${issue.type}">
      <span class="icon">${
        issue.type === 'warning' ? '⚠️' : issue.type === 'error' ? '❌' : 'ℹ️'
      }</span>
...
function getTestFileName(sourceFile: string, language: string): string {
...
function generateRefactoredCode(
  code: string,
  language: string,
  refactorType: string
): string {
 ...
 */`
    ...
    case 'docstring':
      return (
        functions
          .map(
            fn => `def ${fn.name}(${fn.params.map(p => p.name).join(', ')}):
    """
    ${fn.name} - Add description here
...
${
  functions
    .map(
      fn => `### \`${fn.name}(${fn.params.map(p => p.name).join(', ')})\`
...
${
  fn.params
    .map(
      p => `| ${p.name} | ${p.type || 'any'} | Yes | Parameter description |`
    )
    .join('\n') || '| - | - | - | No parameters |'
}
...
function extractFunctionInfo(code: string, language: string): FunctionInfo[] {
  ...
  const pattern = (patterns[language] ?? patterns['javascript']) as RegExp;
  let match;
...
function generateBasicExplanation(
  ...
  const functionMatches =
    code.match(
      /function\s+\w+|def\s+\w+|fn\s+\w+|func\s+\w+|const\s+\w+\s*=\s*(?:async\s*)?\(/g
    ) || [];
  const classMatches = code.match(/class\s+\w+/g) || [];
  const importMatches =
    code.match(/import\s+|from\s+\S+\s+import|require\(/g) || [];
  ...
  if (detailLevel === 'brief') {
    ...
    explanation += `- Code density: ${(
      code.replace(/\s+/g, '').length / lines.length
    ).toFixed(1)} chars/line\n`;
    explanation += `- Nesting depth: ${
      (code.match(/\{/g) || []).length
    } block openings\n`;
  } else {
