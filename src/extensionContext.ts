...
import { AgentModeManager } from './agent/AgentMode';
...
import { AutoApprovalManager } from './safety/AutoApprovalManager';
...
import { SpiralStore } from './stores/spiralStore';
import { TerminalManager } from './terminal/TerminalManager';
import { CoordinationMonitor } from './utils/coordinationUtils';

export class HelixVSCodeExtension {
  ...
  private spiralStore: SpiralStore;
  ...
  get modeManager(): AgentModeManager {
    return this.agentModeManager;
  }

  get approvalManager(): AutoApprovalManager {
    return this.autoApprovalManager;
  }

  get display(): DisplayManager {
    return this.displayManager;
  }

  get contextMgr(): ContextManager {
  ...
  get marketplace(): MarketplaceManager {
    return this.marketplaceManager;
  }

  get checkpoint(): CheckpointManager {
  ...
  constructor(context: vscode.ExtensionContext) {
    ...
    this.authService = new AuthService(this._apiService, context);
    this.sessionStore = new SessionStore(context);
    this.agentStore = new AgentStore();
    this.marketplaceStore = new MarketplaceStore(context);
    this.spiralStore = new SpiralStore(context);
    this.coordinationMonitor = new CoordinationMonitor(
      this._apiService,
      metrics => this.sessionStore.updateCoordinationMetrics(metrics)
    );

    // Initialize managers
    this.configurationManager = ConfigurationManager.getInstance(context);
    this.agentModeManager = AgentModeManager.getInstance(context);
    this.autoApprovalManager = AutoApprovalManager.getInstance(context);
    this.displayManager = DisplayManager.getInstance(context);
    this.contextManager = ContextManager.getInstance(context);
    this.terminalManager = TerminalManager.getInstance(context);
    this.browserManager = BrowserManager.getInstance(context);
    this.marketplaceManager = MarketplaceManager.getInstance(context);
    this.checkpointManager = CheckpointManager.getInstance(context);
    ...
    try {
      ...
      if (apiEndpoint && apiEndpoint !== 'http://localhost:8000') {
        ...
        vscode.window.showInformationMessage(
          'Helix extension connected successfully!'
        );
      } else {
  ...
  private showSetupNotification(): void {
    // If user is already authenticated, just prompt to configure the endpoint.
    if (this.authService.isAuthenticated()) {
      vscode.window
        .showInformationMessage(
          'Helix: set your API endpoint to connect to the platform.',
          'Open Settings'
        )
        .then(sel => {
    ...
    vscode.window
      .showInformationMessage(
        'Welcome to Helix! Sign in to sync memory, agents, and automations across all your tools.',
        'Sign in with GitHub',
        'Sign in with Google',
  ...
  async openAgentPanel(): Promise<void> {
  ...
  async openCoordinationDashboard(): Promise<void> {
    const panel = vscode.window.createWebviewPanel(
      'helixCoordination',
      'Helix Coordination Dashboard',
  ...
  async openSpiralBuilder(): Promise<void> {
    const panel = vscode.window.createWebviewPanel(
      'helixSpiralBuilder',
      'Helix Spiral Builder',
    ...
    const authToken =
      this.context.globalState.get<string>('helix.authTokenValue') ?? '';
  ...
  async executeWorkflow(workflowId: string): Promise<void> {
    try {
      ...
      vscode.window.showInformationMessage(
        `Workflow executed successfully: ${result.status}`
      );
    } catch (error) {
  ...
  async loadConfiguration(): Promise<void> {
    const config = vscode.workspace.getConfiguration('helix');
    this._apiService.setEndpoint(
      config.get('apiEndpoint', 'http://localhost:8000')
    );
    this.websocketService.setEndpoint(
      config.get('websocketEndpoint', 'ws://localhost:8000')
    );
  }
  ...
  private getAgentPanelHtml(webview: vscode.Webview): string {
...
<body>
  ...
  private getMarketplaceHtml(webview: vscode.Webview): string {
    const scriptUri = webview.asWebviewUri(
      vscode.Uri.joinPath(
        this.context.extensionUri,
        'media',
...
<body>
  ...
  private getCoordinationDashboardHtml(webview: vscode.Webview): string {
    const scriptUri = webview.asWebviewUri(
      vscode.Uri.joinPath(
        this.context.extensionUri,
        'media',
...
<body>
  ...
  private getSpiralBuilderHtml(apiBase: string): string {
    const nonce = this.getNonce();
    return `<!DOCTYPE html>
<html lang="en">
<head>
...
window.addEventListener('message', e => {
  ...
  private updateStatusBar(): void {
    ...
    const activeAgents =
      this.agentStore.getActiveAgents().length > 0 ? 'Active' : 'Idle';
