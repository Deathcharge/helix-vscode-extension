...
function getRepository(): any | undefined {
  const git = getGitApi();
  ...
  await vscode.window.withProgress(
    ...
    async () => {
      try {
        ...
        const prompt =
          `Generate a ${commitStyle} commit message for these changes. ` +
        ...
        if (repo) {
          ...
          vscode.window.showInformationMessage(
            'Commit message generated and inserted into the Source Control input box.'
          );
        } else {
    ...
    async () => {
      try {
        ...
        if (!commitLog.trim() && !diffStat.trim()) {
          vscode.window.showWarningMessage(
            `No commits found on ${currentBranch} vs ${baseBranch}.`
          );
          return;
        }

        const prompt =
          `Generate a pull request description in markdown format for branch "${currentBranch}" merging into "${baseBranch}". ` +
...
async function explainGitBlame(
  context: vscode.ExtensionContext
): Promise<void> {
  ...
  await vscode.window.withProgress(
    ...
    async () => {
      try {
        ...
        if (items.length <= 2) {
          vscode.window.showInformationMessage(
            `Line ${lineNumber}: ${explanation.trim()}`
          );
        } else {
...
export function registerGitCommands(context: vscode.ExtensionContext): void {
  context.subscriptions.push(
    vscode.commands.registerCommand('helix.generateCommitMessage', () =>
      generateCommitMessage(context)
    ),
    vscode.commands.registerCommand('helix.generatePRDescription', () =>
      generatePRDescription(context)
    ),
    vscode.commands.registerCommand('helix.explainGitBlame', () =>
      explainGitBlame(context)
    )
  );
}
